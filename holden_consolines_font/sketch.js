// Daniel Holden's Consolines single-stroke font.
// From https://theorangeduck.com/page/debug-draw-text-lines
// Original tables are inlined below.

let myConsolinesFont;

function setup() {
  createCanvas(800, 350); 
  noLoop(); 
  myConsolinesFont = new ConsolinesFont();
}
...
function draw() {
  ...
  let ty = 5;
  let dy = 50; 
  myConsolinesFont.drawString("ABCDEFGHIJKLMNOPQRSTUVWXYZ", 60, ty+=dy, sca, trk);
  myConsolinesFont.drawString("abcdefghijklmnopqrstuvwxyz", 60, ty+=dy, sca, trk);
  myConsolinesFont.drawString("1234567890 ()[]{}<>|~\\", 60, ty+=dy, sca, trk);
  myConsolinesFont.drawString("!@#$%^&*,.?/;:'-+_", 60,ty+=dy, sca, trk); 
  myConsolinesFont.drawString("Hello World!", 60, ty+=dy, sca, trk);
}


function keyPressed(){
  if (key == 's'){
    save("p5_consolines_font.png"); 
  }
}

//=====================================================
// Class to render Holden's Consolines packed line tables
class ConsolinesFont {
  constructor() {
    this.nums = consolines_nums;
    this.offsets = consolines_offsets;
    this.lines = consolines_lines;
    this.unitsPerEm = 128;
    this.charWidth = 64;
  }

  //---------------------------------------------------------
  // Draw a string of text using line segments.
  // tracking: extra pixels added after each glyph (negative tightens spacing)
  drawString(str, x, y, sca, tracking = 0) {
    noFill();
    let cursorX = x;
    const advance = sca * 0.5;
    ...
    for (const chr of str) {
      ...
      if (chr === "\n") {
        ...
        cursorX += 4 * advance + tracking;
      } else if (charCode < 33 || charCode > 126) {
        cursorX += advance + tracking;
      } else {
        this.drawGlyph(chr, cursorX, y, sca);
        cursorX += advance + tracking;
      }
    }
  }
  ...
  //---------------------------------------------------------
  // Draw a single Consolines glyph at the specified position and scale
  drawGlyph(chr, x, y, sca) {
    const ci = chr.charCodeAt(0) - 33;
    const lnum = this.nums[ci];
    const loff = this.offsets[ci];

    for (let li = 0; li < lnum; li++) {
      const packed = this.lines[loff + li];
      const x0 = (packed >>> 0) & 0xFF;
      const y0 = (packed >>> 8) & 0xFF;
      const x1 = (packed >>> 16) & 0xFF;
      const y1 = (packed >>> 24) & 0xFF;

      line(
        x + sca * (    x0 / this.unitsPerEm),
        y + sca * (1 - y0 / this.unitsPerEm),
        x + sca * (    x1 / this.unitsPerEm),
        y + sca * (1 - y1 / this.unitsPerEm)
      );
    }
  }
}

const consolines_nums = [
