// LeonSans font data — converted from ES module format for plain p5.js use.
// Original LeonSans by Jongmin Kim: https://github.com/cmiscm/leonsans
// Conversion: stripped ES module syntax; generateFontData/setCenter produce
// plain JS objects (no Vector class needed at data-load time).

// ─── Shared constants (from src/font/util.js) ────────────────────────────────
const ROTATE_HORIZONTAL = Math.PI;          // 180°
const ROTATE_VERTICAL   = Math.PI / 2;     // 90°
const ROTATE_NONE       = -100;
const VERTEX_GAP        = 3;
const VERTEX_GAP2       = 1.5;
const FONT_HEIGHT       = 824;
const TOFU              = 'tofu';

// ─── Geometry helpers (from src/font/util.js + src/core/paths.js) ────────────
function getR(x1, y1, x2, y2) {
    return -Math.atan2(x1 - x2, y1 - y2);
}

function _bezierTangent(a, b, c, d, t) {
    return 3*(1-t)*(1-t)*(b-a) + 6*(1-t)*t*(c-b) + 3*t*t*(d-c);
}

function getCurveR(x1, y1, x2, y2, x3, y3, x4, y4, t) {
    const dx = _bezierTangent(x1, x2, x3, x4, t);
    const dy = _bezierTangent(y1, y2, y3, y4, t);
    return -Math.atan2(dx, dy);
}

// ─── Font data helpers (adapted from src/font/util.js) ───────────────────────
// setCenter: shifts all vertex coords so the glyph is centered at (0,0),
// and returns plain objects {type, x, y, x2, y2, x3, y3, ratio}.
function _setCenter(arr, fw, fh) {
    const cx = fw / 2, cy = fh / 2;
    const result = [];
    for (const mp of arr) {
        const type = mp[0];
        const obj = { type, x: mp[1] - cx, y: mp[2] - cy,
                      x2: 0, y2: 0, x3: 0, y3: 0, ratio: {} };
        let r;
        if (type === 'b') {
            obj.x2 = mp[3] - cx; obj.y2 = mp[4] - cy;
            obj.x3 = mp[5] - cx; obj.y3 = mp[6] - cy;
            r = mp[7] || {};
        } else {
            r = mp[3] || {};
        }
...
// generateFontData: creates a character definition with pre-centered vertices.
function generateFontData(w, fw, fh, x1, x2, y1, y2, paths) {
    const p = paths.map(path => ({
        d: path.d,
        v: _setCenter(path.v, fw, fh),
    }));
    const obj = {
        rect:  { w, h: FONT_HEIGHT, fw, fh },
        ratio: { x1, x2, y1, y2 },
        p,
        clone() {
            return {
                ...
                p: obj.p.map(path => ({
                    d: path.d,
                    v: path.v.map(v => ({ ...v, ratio: { ...v.ratio } })),
                })),
            };
        },
    };
...
// ─── Font data (UPPER / LOWER / NUMBER / SPECIAL) ────────────────────────────
...
// ─── Final font map ──────────────────────────────────────────────────────────
const LEON_FONT = Object.assign({}, UPPER, LOWER, NUMBER, SPECIAL);
